...
export default function Settings() {
  ...
  const [currency, setCurrency] = useState(restaurant.currency || 'CHF')
  const [panelLang, setPanelLang] = useState(lang)
  ...
  function handlePanelLangChange(code) {
    setPanelLang(code)
    changeLang(code)
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16, alignItems: 'start' }}>
        ...
        <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: T.rCard, padding: 24 }}>
          <h2 style={{ fontFamily: T.syne, fontWeight: 700, fontSize: 13, textTransform: 'uppercase', letterSpacing: 0.5, color: T.textSecondary, margin: '0 0 18px' }}>
            {t('settings.yourBrand')}
          </h2>
          <Field label={t('onboarding.venueName')}>
            <input style={inputStyle} value={name} onChange={e => setName(e.target.value)} />
          </Field>
          <Field label={t('settings.primaryColor')} hint={t('settings.colorHint')}>
          ...
          <Field label={t('settings.panelLanguage')}>
            <select value={panelLang} onChange={e => handlePanelLangChange(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
              {LANGUAGES.map(l => (
                <option key={l.code} value={l.code}>{l.flag} {l.label}</option>
              ))}
            </select>
          </Field>
          <Field label={t('settings.requireConfirmation')}>
            <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
              ...
              <div>
                ...
                <span style={{ fontFamily: T.syne, fontSize: 12, color: T.textSecondary, display: 'block', marginTop: 3 }}>
                  {t('settings.requireConfirmationHint')}
                </span>
              </div>
            </label>
          </Field>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
            <Button onClick={save} disabled={busy} style={{ opacity: busy ? 0.6 : 1 }}>{busy ? t('common.loading') : t('settings.saveChanges')}</Button>
            {saved && <span style={{ fontFamily: T.syne, fontSize: 13, color: T.green, fontWeight: 700 }}>✓ {t('settings.saved')}</span>}
          </div>
        </div>
        ...
        <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: T.rCard, padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ width: '100%', background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.rSection, padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
              ...
              <button onClick={copyLink} style={{ border: 'none', background: 'none', cursor: 'pointer', fontFamily: T.syne, fontSize: 11, fontWeight: 700, color: T.primary, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{t('settings.copy')}</button>
            </div>
            <div style={{ display: 'flex', gap: 8, width: '100%' }}>
              <Button onClick={() => downloadQR('png')} style={{ flex: 1 }}>{t('settings.downloadPng')}</Button>
